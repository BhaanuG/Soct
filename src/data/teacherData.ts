...
export function generateTeacherData(teacherId: string, teacherName: string, specialization: string[]) {
  // Use teacher ID to generate consistent random values
  const seed = parseInt(teacherId.slice(-2));

  const subjectMapping: { [key: string]: any } = {
...
function generateWeekData(subjects: any[], seed: number, weekIndex: number) {
  const data: any = {};
