...
export function GradebookView({ teacherData }: GradebookViewProps) {
  // Generate sample student grades based on teacher's courses
  const generateStudentGrades = (subject: any) => {
    const students = [];
    for (let i = 0; i < Math.min(10, subject.enrolledStudents); i++) {
      ...
      students.push({
        ...
        assignments: subject.recentAssignments.map((a: any, idx: number) => ({
          name: a.name,
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      {subject.recentAssignments.map((a: any, idx: number) => (
