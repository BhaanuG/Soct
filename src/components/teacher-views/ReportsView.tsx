...
export function ReportsView({ teacherData }: ReportsViewProps) {
  const seed = parseInt(teacherData.lecturer.id.slice(-2));

  // Prepare analytics data
  const performanceDistribution = [
    { grade: 'A (90-100)', count: Math.round(teacherData.subjects.reduce((sum: number, s: any) => sum + s.enrolledStudents, 0) * 0.15) },
    { grade: 'B (80-89)', count: Math.round(teacherData.subjects.reduce((sum: number, s: any) => sum + s.enrolledStudents, 0) * 0.35) },
    { grade: 'C (70-79)', count: Math.round(teacherData.subjects.reduce((sum: number, s: any) => sum + s.enrolledStudents, 0) * 0.30) },
    { grade: 'D (60-69)', count: Math.round(teacherData.subjects.reduce((sum: number, s: any) => sum + s.enrolledStudents, 0) * 0.15) },
    { grade: 'F (<60)', count: Math.round(teacherData.subjects.reduce((sum: number, s: any) => sum + s.enrolledStudents, 0) * 0.05) }
  ];

  const attendanceData = teacherData.subjects.map((subject: any) => ({
    course: subject.name.split(' ')[0],
    attendance: subject.completionRate,
    target: 85
  }));

  const outcomeData = teacherData.subjects[0].outcomes.map((outcome: any) => ({
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <Card className="p-4">
          ...
          <div className="text-2xl">
            {teacherData.subjects.reduce((sum: number, s: any) => sum + s.enrolledStudents, 0)}
          </div>
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl">
            {Math.round(teacherData.subjects.reduce((sum: number, s: any) => sum + s.avgGrade, 0) / teacherData.subjects.length)}%
          </div>
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl">
            {Math.round(teacherData.subjects.reduce((sum: number, s: any) => sum + s.completionRate, 0) / teacherData.subjects.length)}%
          </div>
      ...
      <Tabs defaultValue="performance" className="w-full">
        ...
        <TabsContent value="outcomes" className="mt-4 space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {teacherData.subjects.map((subject: any, subIndex: number) => (
              <Card key={subIndex} className="p-4">
                <h4 className="mb-3">{subject.name}</h4>
                <div className="space-y-2">
                  {subject.outcomes.map((outcome: any, oIndex: number) => (
