...
interface TeacherDashboardViewProps {
  teacherData: any;
  onTotalStudentsClick: () => void;
  onPendingReviewsClick: () => void;
  onAvgCompletionClick: () => void;
}

export function TeacherDashboardView({ teacherData, onTotalStudentsClick, onPendingReviewsClick, onAvgCompletionClick }: TeacherDashboardViewProps) {
  return (
      ...
      <div className="grid md:grid-cols-4 gap-4">
        ...
        <Card className="p-4 cursor-pointer hover:bg-accent/50 transition-colors" onClick={onTotalStudentsClick}>
          ...
          <div className="text-2xl">
            {teacherData.subjects.reduce((sum: number, subject: any) => sum + subject.enrolledStudents, 0)}
          </div>
        ...
        <Card className="p-4 cursor-pointer hover:bg-accent/50 transition-colors" onClick={onAvgCompletionClick}>
          ...
          <div className="text-2xl">
            {Math.round(teacherData.subjects.reduce((sum: number, subject: any) => sum + subject.completionRate, 0) / teacherData.subjects.length)}%
          </div>
        ...
        <Card className="p-4 cursor-pointer hover:bg-accent/50 transition-colors" onClick={onPendingReviewsClick}>
          ...
          <div className="text-2xl">
            {teacherData.subjects.reduce((sum: number, subject: any) => 
              sum + subject.recentAssignments.reduce((assignSum: number, assign: any) => assignSum + assign.pending, 0), 0
            )}
          </div>
            ...
            <TabsContent key={subject.id} value={subject.id} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {subject.outcomes.map((outcome: any, index: number) => (
              ...
              <div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {subject.recentAssignments.map((assignment: any, index: number) => (
                      <TableRow key={index}>
