...
export function PendingReviewsModal({ isOpen, onClose, subjects }: PendingReviewsModalProps) {
  ...
  const pendingAssignments = subjects.flatMap(subject => 
    subject.recentAssignments
      .filter((assignment: any) => assignment.pending > 0)
      .map((assignment: any) => ({
        ...assignment,
  ...
  const getPriorityColor = (pending: number, total: number) => {
  ...
  const getDaysOverdue = (submittedDate: string) => {
