...
export function CalendarView({ studentData }: CalendarViewProps) {
  // Collect all assignments with dates
  const allEvents = studentData.subjects.flatMap((subject: any) =>
  ...
  const upcomingEvents = allEvents.filter((e: any) => e.status === 'pending' || e.status === 'in-progress');
  const completedEvents = allEvents.filter((e: any) => e.status === 'graded' || e.status === 'submitted');
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-4">
          ...
          <div className="space-y-3">
            {completedEvents.slice(0, 5).map((event: any, index: number) => (
