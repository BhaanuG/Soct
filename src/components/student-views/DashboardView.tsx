...
export function DashboardView({ studentData, onEnrolledCoursesClick, onRecentActivityClick }: DashboardViewProps) {
  ...
  const subjectEngagementData = studentData.subjects.map((subject: any) => ({
  ...
  const courseAttendanceData = studentData.subjects.map((subject: any) => ({
    course: subject.name,
    attendance: subject.attendance,
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Card className="p-4">
          ...
          <div className="space-y-2">
            <Progress value={Math.round(studentData.subjects.reduce((sum: number, s: any) => sum + s.progress, 0) / studentData.subjects.length)} />
            <p className="text-sm text-muted-foreground">
              {Math.round(studentData.subjects.reduce((sum: number, s: any) => sum + s.progress, 0) / studentData.subjects.length)}% Average Progress
            </p>
          </div>
        </Card>
