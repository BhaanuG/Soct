...
export function AssignmentsView({ studentData }: AssignmentsViewProps) {
  // Collect all assignments from all subjects
  const allAssignments = studentData.subjects.flatMap((subject: any) =>
    subject.recentAssignments.map((assignment: any) => ({
      ...assignment,
  ...
  const inProgressAssignments = allAssignments.filter((a: any) => a.status === 'in-progress' || a.status === 'submitted');
  const gradedAssignments = allAssignments.filter((a: any) => a.status === 'graded');
      ...
      <div className="grid md:grid-cols-4 gap-4">
        ...
        <Card className="p-4">
          ...
          <div className="text-2xl">
            {Math.round(gradedAssignments.reduce((sum: number, a: any) => sum + (a.score || 0), 0) / (gradedAssignments.length || 1))}%
          </div>
        ...
        <TabsContent value="progress" className="mt-4">
          <Card className="p-4">
            ...
            <div className="space-y-3">
              ...
              {inProgressAssignments.map((assignment: any, index: number) => (
        ...
        <TabsContent value="graded" className="mt-4">
          <Card className="p-4">
            ...
            <Table>
              ...
              <TableBody>
                {gradedAssignments.map((assignment: any, index: number) => (
                  <TableRow key={index}>
        ...
        <TabsContent value="all" className="mt-4">
          <Card className="p-4">
            ...
            <Table>
              ...
              <TableBody>
                {[...additionalAssignments, ...allAssignments].map((assignment: any, index: number) => (
                  <TableRow key={index}>
                    <TableCell>{assignment.name}</TableCell>
                    <TableCell>{assignment.courseName}</TableCell>
                    <TableCell>{assignment.date}</TableCell>
