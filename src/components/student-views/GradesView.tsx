...
export function GradesView({ studentData }: GradesViewProps) {
  return (
      ...
      <Card className="p-4">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          <div className="text-center p-3 bg-blue-900/20 rounded">
            <div className="text-2xl">{Math.round(studentData.subjects.reduce((sum: number, s: any) => sum + s.grade, 0) / studentData.subjects.length)}%</div>
            <div className="text-sm text-muted-foreground">Average Grade</div>
          </div>
          <div className="text-center p-3 bg-green-900/20 rounded">
            <div className="text-2xl">{studentData.subjects.filter((s: any) => s.grade >= 80).length}</div>
